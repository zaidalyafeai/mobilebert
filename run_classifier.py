...
def build_classifier_serving_input_fn(seq_length):
...
class LiteRunner(object):
  ...
  def evaluate(self, input_fn, params):
    ...
    for feature in generate_features():
      ...
      probabilities = self.run(input_ids, input_mask, segment_ids)
      predictions.append(np.argmax(probabilities))
      labels.append(feature["label_ids"][0])
...
def main(_):
  ...
  if (not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and
      not FLAGS.export_dir):
    raise ValueError(
        "At least one of `do_train`, `do_eval` or `do_predict' must be True, "
        "or export_dir should be specified.")
  ...
  if FLAGS.tfrecord_dir is not None:
  ...
  if FLAGS.do_train:
    train_file = os.path.join(
        FLAGS.tfrecord_dir or FLAGS.output_dir, "train.tf_record")
  ...
  if FLAGS.do_eval:
    ...
    eval_file = os.path.join(
        FLAGS.tfrecord_dir or FLAGS.output_dir, "eval.tf_record")
  ...
  if FLAGS.do_predict:
    ...
    predict_file = os.path.join(
        FLAGS.tfrecord_dir or FLAGS.output_dir, "predict.tf_record")
  ...
  if FLAGS.export_dir is not None:
    serving_input_fn = (build_classifier_serving_input_fn(FLAGS.max_seq_length))
